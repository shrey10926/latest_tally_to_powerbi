#get xml of all vouchers of specific date
#do the required changes in xml_flie/voucher_fetch.xml
...
for voucher_type in ['Purchase Vouchers', 'Sales Vouchers']:
    ...
    for elem in root.iter('SVFROMDATE'):
        elem.text = '20200101' #datetime.today().strftime('%Y%m%d')
    ...
    xml_file = open(f"client_files/{voucher_type}.xml", "w", newline='')
...
try:
  ...
  for voucher_type in ['Sales Vouchers']:
    ...
    with open(f'client_files/{voucher_type}.xml', 'r') as f:
      # data = xmltodict.parse(f.read())['ENVELOPE']['BODY']['IMPORTDATA']['REQUESTDATA']['TALLYMESSAGE']
      tree = et.parse('client_files\Sales Vouchers.xml')
